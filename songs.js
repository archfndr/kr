window.ARCHFNDR_TRACKS = [
  {
    id: 1,
    slug: 'lost',
    title: 'Lost',
    artist: 'Frank Ocean',
    album: 'channel ORANGE',
    genres: ['R&B', 'ALT R&B'],
    cover: 'https://coverartarchive.org/release/a942130d-bf15-4a5b-9903-a2bd69792e0a/front-500',
    comment: { en: 'Love lost', ko: '잃어버린 사랑' },
    description: {
      en: 'Bright on the surface, restless underneath. The kind of song that keeps moving even when the feeling inside it does not.',
      ko: '겉은 밝지만 안쪽은 계속 흔들린다. 감정은 멈춰 있는데 노래만 앞으로 나아가는 듯한 곡.'
    },
    spotify: 'https://open.spotify.com/track/3GZD6HmiNUhxXYf8Gch723',
    apple: 'https://music.apple.com/us/song/lost/1440766784',
    youtube: 'https://www.youtube.com/results?search_query=Frank+Ocean+Lost'
  },
  {
    id: 2,
    slug: 'alesis',
    title: 'Alesis',
    artist: 'Mk.gee',
    album: 'Two Star & The Dream Police',
    genres: ['ALT', 'NEO-PSYCH'],
    cover: 'https://coverartarchive.org/release/ec873d45-a37c-4738-b817-36d8cc3591e9/front-500',
    comment: { en: 'Blurred edges, sharp feeling.', ko: '흐릿한 질감 속에 또렷하게 남는 감정.' },
    description: {
      en: 'A warped, close-up kind of song — damaged textures held together by something strangely tender.',
      ko: '뒤틀린 질감과 가까운 거리감이 묘하게 다정한 감정으로 이어지는 곡.'
    },
    spotify: 'https://open.spotify.com/search/Mk.gee%20Alesis',
    apple: 'https://music.apple.com/us/search?term=Mk.gee%20Alesis',
    youtube: 'https://www.youtube.com/results?search_query=Mk.gee+Alesis'
  },
  {
    id: 3,
    slug: 'baby',
    title: 'Baby',
    artist: 'Dijon',
    album: '—',
    genres: ['R&B', 'SOUL'],
    cover: window.ARCHFNDR_MEDIA?.baby || '',
    tone: 'sand',
    comment: { en: 'Joy caught mid-motion.', ko: '움직이는 순간 그대로 붙잡힌 기쁨.' },
    description: {
      en: 'Loose, physical, and alive — the room matters just as much as the song itself.',
      ko: '느슨하고 육체적이며 살아 있다. 곡 자체만큼 그 공간의 공기까지 중요하게 느껴지는 음악.'
    },
    spotify: 'https://open.spotify.com/search/Dijon%20Baby',
    apple: 'https://music.apple.com/us/search?term=Dijon%20Baby',
    youtube: 'https://www.youtube.com/results?search_query=Dijon+Baby'
  },
  {
    id: 4,
    slug: 'self-control',
    title: 'Self Control',
    artist: 'Frank Ocean',
    album: 'Blonde',
    genres: ['R&B', 'ART POP'],
    cover: 'https://coverartarchive.org/release/8294645a-f996-44b6-9060-7f189b9f59f3/front-500',
    comment: { en: 'Memory stretched thin.', ko: '기억이 얇게 늘어나는 순간.' },
    description: {
      en: 'A memory slowly losing its edges until it becomes atmosphere.',
      ko: '기억의 윤곽이 천천히 사라져 결국 분위기만 남는 곡.'
    },
    spotify: 'https://open.spotify.com/search/Frank%20Ocean%20Self%20Control',
    apple: 'https://music.apple.com/us/search?term=Frank%20Ocean%20Self%20Control',
    youtube: 'https://www.youtube.com/results?search_query=Frank+Ocean+Self+Control'
  },
  {
    id: 5,
    slug: 'luvsic',
    title: 'Luv(sic)',
    artist: 'Nujabes',
    album: 'Modal Soul',
    genres: ['HIP-HOP', 'JAZZ RAP'],
    cover: 'https://coverartarchive.org/release/9f3a4a9b-5741-4a3b-9350-10940ce8bbf3/front-500',
    comment: { en: 'A feeling that never ages.', ko: '시간이 지나도 늙지 않는 감정.' },
    description: {
      en: 'Dusty drums and daylight through curtains. Familiar without ever becoming dull.',
      ko: '먼지 낀 드럼과 커튼 사이의 햇빛. 익숙해져도 지루해지지 않는 음악.'
    },
    spotify: 'https://open.spotify.com/search/Nujabes%20Luvsic',
    apple: 'https://music.apple.com/us/search?term=Nujabes%20Luvsic',
    youtube: 'https://www.youtube.com/results?search_query=Nujabes+Luvsic'
  },
  {
    id: 6,
    slug: 'like-you-do',
    title: 'Like You Do',
    artist: 'Joji',
    album: 'Nectar',
    genres: ['R&B', 'POP'],
    cover: 'https://coverartarchive.org/release/92abd49f-0a1a-46dc-a1f8-65593b9e7123/front-500',
    comment: { en: 'Quiet devastation.', ko: '조용한 붕괴.' },
    description: {
      en: 'Melancholy without the melodrama. Close, quiet, replayable.',
      ko: '과장 없는 우울. 가까이 있고, 조용하고, 계속 다시 듣게 되는 곡.'
    },
    spotify: 'https://open.spotify.com/search/Joji%20Like%20You%20Do',
    apple: 'https://music.apple.com/us/search?term=Joji%20Like%20You%20Do',
    youtube: 'https://www.youtube.com/results?search_query=Joji+Like+You+Do'
  },
  {
    id: 7,
    slug: 'quadeca',
    title: 'A strange little world',
    artist: 'Quadeca',
    album: 'Vanisher, Horizon Scraper',
    genres: ['ART POP', 'FOLKTRONICA'],
    cover: 'https://coverartarchive.org/release/1c25b84b-e51e-47cc-8c88-e01ff28f8740/front-500',
    comment: { en: 'Leave the seams exposed.', ko: '봉제선을 숨기지 않는 음악.' },
    description: {
      en: 'For when polished music feels too clean and you want the construction left visible.',
      ko: '매끈한 음악이 너무 깨끗하게 느껴질 때, 만드는 흔적까지 그대로 드러난 음악.'
    },
    spotify: 'https://open.spotify.com/search/Quadeca',
    apple: 'https://music.apple.com/us/search?term=Quadeca',
    youtube: 'https://www.youtube.com/results?search_query=Quadeca'
  },
  {
    id: 8,
    slug: 'pyramids',
    title: 'Pyramids',
    artist: 'Frank Ocean',
    album: 'channel ORANGE',
    genres: ['R&B', 'ELECTRONIC'],
    cover: 'https://coverartarchive.org/release/a942130d-bf15-4a5b-9903-a2bd69792e0a/front-500',
    comment: { en: 'A whole night in one song.', ko: '한 곡 안에 들어 있는 밤 전체.' },
    description: {
      en: 'Two halves, two temperatures, one long fall into the same story.',
      ko: '서로 다른 두 온도가 결국 하나의 이야기로 떨어지는 긴 곡.'
    },
    spotify: 'https://open.spotify.com/search/Frank%20Ocean%20Pyramids',
    apple: 'https://music.apple.com/us/search?term=Frank%20Ocean%20Pyramids',
    youtube: 'https://www.youtube.com/results?search_query=Frank+Ocean+Pyramids'
  },
  {
    id: 9,
    slug: 'metalmind',
    title: 'Metalmind',
    artist: 'Dijon',
    album: 'Absolutely',
    genres: ['R&B', 'EXPERIMENTAL'],
    cover: '',
    tone: 'violet',
    comment: { en: 'Beautifully unstable.', ko: '아름답게 불안정한 곡.' },
    description: {
      en: 'Feels like it could collapse at any second, which is exactly why it stays alive.',
      ko: '언제든 무너질 것 같아서 오히려 끝까지 살아 있는 듯한 곡.'
    },
    spotify: 'https://open.spotify.com/search/Dijon%20Metalmind',
    apple: 'https://music.apple.com/us/search?term=Dijon%20Metalmind',
    youtube: 'https://www.youtube.com/results?search_query=Dijon+Metalmind'
  },
  {
    id: 10,
    slug: 'ivy',
    title: 'Ivy',
    artist: 'Frank Ocean',
    album: 'Blonde',
    genres: ['R&B', 'INDIE'],
    cover: 'https://coverartarchive.org/release/8294645a-f996-44b6-9060-7f189b9f59f3/front-500',
    comment: { en: 'Youth after it is already gone.', ko: '이미 지나가 버린 뒤의 젊음.' },
    description: {
      en: 'A song that sounds like remembering something while it is disappearing.',
      ko: '사라지는 동시에 기억하고 있는 듯한 곡.'
    },
    spotify: 'https://open.spotify.com/search/Frank%20Ocean%20Ivy',
    apple: 'https://music.apple.com/us/search?term=Frank%20Ocean%20Ivy',
    youtube: 'https://www.youtube.com/results?search_query=Frank+Ocean+Ivy'
  },
  {
    id: 11,
    slug: 'wallflower',
    title: 'Wallflower',
    artist: 'Alex Sampson',
    album: '—',
    genres: ['POP', 'INDIE POP'],
    cover: '',
    tone: 'blue',
    comment: { en: 'Soft focus.', ko: '초점이 살짝 흐려진 듯한 감정.' },
    description: {
      en: 'Clean enough to feel immediate, soft enough to stay around.',
      ko: '바로 와닿을 만큼 선명하지만 오래 남을 만큼 부드러운 곡.'
    },
    spotify: 'https://open.spotify.com/search/Alex%20Sampson%20Wallflower',
    apple: 'https://music.apple.com/us/search?term=Alex%20Sampson%20Wallflower',
    youtube: 'https://www.youtube.com/results?search_query=Alex+Sampson+Wallflower'
  },
  {
    id: 12,
    slug: 'sunday-girl',
    title: 'Sunday Girl',
    artist: 'Devvon Terrell',
    album: '—',
    genres: ['R&B', 'POP'],
    cover: '',
    tone: 'green',
    comment: { en: 'Easy light.', ko: '편안한 빛 같은 곡.' },
    description: {
      en: 'Warm, direct, and easy to keep close without asking for too much.',
      ko: '따뜻하고 직접적이며 부담 없이 곁에 두기 좋은 곡.'
    },
    spotify: 'https://open.spotify.com/search/Devvon%20Terrell%20Sunday%20Girl',
    apple: 'https://music.apple.com/us/search?term=Devvon%20Terrell%20Sunday%20Girl',
    youtube: 'https://www.youtube.com/results?search_query=Devvon+Terrell+Sunday+Girl'
  }
];
